    ...
    if (props.tunnelOptions) {
      ...
      props.tunnelOptions.forEach((options, index) => {
        ...
        if (options.tunnelInsideCidr) {
          ...
          if (!/^10\.1\.\d{1,3}\.\d{1,3}\/30$/.test(options.tunnelInsideCidr)) {
            /* eslint-disable-next-line max-len */
            throw new Error(`The \`tunnelInsideCidr\` ${options.tunnelInsideCidr} for tunnel ${index + 1} is not a size /30 CIDR block from the 10.1.0.0/22 range.`);
          }
        }
      });
    }
